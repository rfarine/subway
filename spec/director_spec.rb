...
describe 'Director' do
	...
	describe '## ride_on_l(start,final)' do
		it { subject.should respond_to(:ride_on_l).with(2).argument }

		it 'should direct the user towards Manhattan' do
			subject.ride_on_l(lorimer,eighth_ave).should eq("towards Manhattan")
		end

		it 'should direct the user towards Brooklyn' do
			subject.ride_on_l(eighth_ave,lorimer).should eq("towards Brooklyn")
		end

		it 'should raise an error, because they used identical starting and final stops' do
			lambda {
				subject.ride_on_l(eighth_ave,eighth_ave)
			}.should raise_error
		end
	end

	describe '## ride_on_f(start,final)' do
		it { subject.should respond_to(:ride_on_f).with(2).argument }

		it 'should direct the user uptown' do
			subject.ride_on_f(second_ave,west_fourth).should eq("uptown")
		end

		it 'should direct the user downtown' do
			subject.ride_on_f(west_fourth,second_ave).should eq("downtown")
		end

		it 'should raise an error, because they used identical starting and final stops' do
			lambda {
				subject.ride_on_f(west_fourth,west_fourth)
			}.should raise_error
		end
	end
	...
	describe '## give_directions(start,final,direction,number_of_stops)' do
		...
		it 'should give directions on the L from Lorimer to 8th Ave, with 6 stops' do
			direction = subject.ride_on_l(lorimer,eighth_ave)
		...
		it 'should give directions on the F from Second Ave to West Fourth, with 2 stops' do
			direction = subject.ride_on_f(second_ave,west_fourth)
